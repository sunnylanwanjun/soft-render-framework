...
#define LIGHT_NUM             4
Terrain3D_Material::Terrain3D_Material(){
...
Terrain3D_Material* Terrain3D_Material::create(){
	Terrain3D_Material* terrain = new Terrain3D_Material;
	terrain->autoRelease();
	terrain->init();
...
bool Terrain3D_Material::init(){
	...
	obj_tank = MaterialNode3D::createWithPlgfile("tank3.plg");
	...
	obj_player = MaterialNode3D::createWithCobfile("tie04.cob");
	obj_player->setScale(Vec3f(3.0f,3.0f,3.0f));
	obj_player->retain();

	obj_marker = MaterialNode3D::createWithPlgfile("marker2.plg");
	obj_marker->setScale(Vec3f(3.0f,3.0f,3.0f));
	obj_marker->retain();
	...
	obj_tower = MaterialNode3D::createWithPlgfile("towerg1.plg");
	obj_tower->setScale(Vec3f(1.0f,2.0f,1.0f));
	obj_tower->retain();
...
void Terrain3D_Material::update( float dt ){
	rend_list->reset();
	...
	static Vec3f playerDir;
	playerDir=cam->dir.v3f();
	playerDir.y+=dPI2;
	obj_player->setRotation(playerDir);
